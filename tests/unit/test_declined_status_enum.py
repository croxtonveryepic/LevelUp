...
class TestDeclinedStatusPersistence:
    ...
    def test_read_declined_ticket_from_db(self, tmp_path: Path):
        """read_tickets() should correctly return declined tickets from DB."""
        add_ticket(tmp_path, "Rejected Feature", "Not needed anymore")
    ...
    def test_declined_status_round_trip(self, tmp_path: Path):
        """Declined status should survive a read/write round trip."""
        add_ticket(tmp_path, "My Task", "Description")
        set_ticket_status(tmp_path, 1, TicketStatus.DECLINED)
