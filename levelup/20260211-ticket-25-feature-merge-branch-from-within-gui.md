...
- **Affected files:** src/levelup/agents/merge.py, src/levelup/agents/__init__.py, src/levelup/gui/run_terminal.py, src/levelup/gui/ticket_detail.py, tests/unit/test_merge_agent.py, tests/unit/test_run_terminal_merge_button.py, tests/integration/test_merge_workflow.py
- **Risks:**
...
- [INFO] `src/levelup/agents/merge.py`: MergeAgent class lacks type hints on __init__ for project_path parameter (should be Path, not left implicit).
...
- [INFO] `src/levelup/agents/__init__.py`: MergeAgent is exported in __all__ but other agents (ReconAgent, SecurityAgent, etc.) are not. Inconsistent public API.
