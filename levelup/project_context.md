...
- **TerminalEmulatorWidget** (`src/levelup/gui/terminal_emulator.py`):
    - Full VT100 terminal emulator using `pyte` for screen state management
    - Uses `pyte.HistoryScreen` with 10,000 line history buffer
    - QPainter-based rendering for cell-by-cell display
    - Supports two color schemes: `CatppuccinMochaColors` (dark) and `LightTerminalColors` (light)
    - Cross-platform PTY backend using `pywinpty` (Windows) or `ptyprocess` (Unix)
- **Current scrollback implementation**:
    - `_scroll_offset` tracks lines scrolled up from bottom (0 = at bottom)
    - `wheelEvent()` updates `_scroll_offset` (±3 lines per event)
    - Scroll offset clamped to `len(screen.history.top)` (history length)
    - `keyPressEvent()` snaps to bottom on input (`_scroll_offset = 0`)
    - Cursor hidden when `_scroll_offset > 0`
- **paintEvent() behavior (CURRENT ISSUE)**:
    - Always renders `screen.buffer[row]` for rows 0 to `_rows-1`
    - Does NOT composite history lines when `_scroll_offset > 0`
    - This means scrolling up updates the offset but viewport still shows current buffer
    - History lines in `screen.history.top` deque are never accessed in paintEvent
- **pyte.HistoryScreen structure**:
    - `screen.buffer` - current visible buffer (list of lines)
    - `screen.history.top` - deque of historical lines (lines that scrolled off top)
    - `screen.history.bottom` - deque for alternate screen buffer (not used in normal mode)
    - History lines are the same structure as buffer lines (list of Char objects)
    - History order: `history.top[0]` is oldest, `history.top[-1]` is newest (most recently pushed)
- **Rendering flow**:
    - Each cell is a `pyte.Char` with `.data`, `.fg`, `.bg`, `.bold`, `.reverse` attributes
    - Colors resolved through `CatppuccinMochaColors.resolve()` or `LightTerminalColors.resolve()`
    - Selection highlighting handled per-cell with `_cell_in_selection()` check
    - Cursor drawn separately at `(screen.cursor.x, screen.cursor.y)` when `_scroll_offset == 0`
